    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">Gestiona los usuarios aprobados del sistema</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {users.filter(u => u.role !== 'admin' && u.status === 'approved').map(u => (
              <tr key={u.uid} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4 text-right">
                  {u.role !== 'admin' && (
                    <button onClick={() => handleReject(u.uid)} className="px-3 py-1 text-xs font-bold text-red-600 hover:bg-red-50 rounded border border-red-200 transition-colors">
                      Revocar
                    </button>
                  )}
                  {u.role !== 'admin' && (
            ...
            <div className="flex-1 overflow-auto p-4 bg-slate-50 flex items-center justify-center min-h-[400px]">
              {selectedFile.url.startsWith('data:image/') ? (
                ...
                <div className="text-center p-12">
                  ...
                  <p className="text-slate-500 mb-6">Este archivo no se puede previsualizar directamente.</p>
                  <a 
